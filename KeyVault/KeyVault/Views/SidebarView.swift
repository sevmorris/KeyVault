...
struct SidebarView: View {
    ...
    /// The types KeyVault holds outright — the ones it can back up, restore
    /// and rewrite, and the reason the app exists.
    private var secretTypes: [KeyType] {
        ordered(KeyType.allCases.filter(\.isStored))
    }
    ...
    private var presentKeyTypes: [KeyType] {
        ordered(KeyType.allCases.filter { !$0.isStored })
            .filter { viewModel.keyCount(for: $0) > 0 }
    }

    /// Preferred display order. The two groups are derived from `isStored`
    ...
    private func ordered(_ types: [KeyType]) -> [KeyType] {
        let preferred: [KeyType] = [.note, .api, .file, .ssh, .gpg, .age]
        return types.sorted {
